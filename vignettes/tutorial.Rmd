...
- producte_view_cnt_cat : product view count of user on date_id
```{r setup}
library(dplyr)
library(rollup)
...
- `grouping_sets((a,b),a,b, NA)` is equivalent to row binding of `group_by(a,b)`, `group_by(a)`, `group_by(b)` and without `group_by` operation. 
```{r warning=FALSE}
library(tidyr)
